...
class LessonCRUDTestCase(APITestCase):
    def setUp(self):
        ...
        self.course = Course.objects.create(
            title="Test Course", description="Test Description", owner=self.user1
        )
    ...
    def test_update_lesson_with_valid_link(self):
        ...
        data = {"title": "Updated Lesson", "video_url": "https://youtu.be/newvideo123"}

        response = self.client.patch(
            reverse("lesson-detail", args=[self.lesson1.id]), data
        )
...
class SubscriptionTestCase(APITestCase):
    ...
    def test_subscribe_to_course(self):
        ...
        self.assertTrue(
            Subscription.objects.filter(
                user=self.user1, course=self.course, is_active=True
            ).exists()
        )
    ...
    def test_subscription_status_in_course_response(self):
        ...
        # Создаем подписку
        Subscription.objects.create(user=self.user1, course=self.course, is_active=True)
    ...
    def test_cannot_subscribe_twice(self):
        ...
        # Первая подписка
        response1 = self.client.post(reverse("course-subscribe", args=[self.course.id]))
        self.assertEqual(response1.status_code, status.HTTP_201_CREATED)

        # Вторая попытка подписки
        response2 = self.client.post(reverse("course-subscribe", args=[self.course.id]))
        self.assertEqual(
            response2.status_code, status.HTTP_201_CREATED
        )  # Должно обновить существующую
...
class CoursePaginationTestCase(APITestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            email="user@test.com", password="testpass123", is_active=True
        )
    ...
    def test_course_pagination(self):
        ...
        )  # 8 курсов, page_size=5, на второй странице 3
